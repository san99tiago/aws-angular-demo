...
export class ChatAgentComponent implements OnInit, AfterViewChecked {
	...
	ngOnInit(): void {
		...
		this.chats = [{ "name": 'BAES', "lema": "Aprendamos AWS para el Party!", "hi": "Soy un frontend sencillo desplegado en AWS!", "image": "/chatAgent/bot.png" },
		]
	...
	public async generarRecomendacion() {
		...
		// Get time and update result with it
		const now = new Date();
		const hour = now.getHours();
		const minutes = now.getMinutes();
		const seconds = now.getSeconds();
		let result = `Saludos a las ${hour}:${minutes}:${seconds}`;
